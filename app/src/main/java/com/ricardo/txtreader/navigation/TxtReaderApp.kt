...
fun TxtReaderApp() {
    ...
    var sleepTimerTargetMillis by remember { mutableStateOf<Long?>(null) }
    ...
    fun stopSpeech(clearTimer: Boolean = true, savePosition: Boolean = true) {
    ...
    fun startReadingText(text: String, startOffset: Int = 0) {
        ...
        stopSpeech(clearTimer = false, savePosition = false)
        speechSessionId += 1
    ...
    LaunchedEffect(sleepTimerTargetMillis) {
        val target = sleepTimerTargetMillis ?: return@LaunchedEffect
        val waitMs = target - System.currentTimeMillis()
        if (waitMs > 0) delay(waitMs)
        if (sleepTimerTargetMillis == target) {
            stopSpeech(clearTimer = true, savePosition = true)
            ttsMessage = "Autoapagado"
        }
    }
        ...
        composable(Destination.Reader.route) {
            ReaderScreen(
                ...
                onSetSleepTimer = { minutes ->
                    ...
                    sleepTimerTargetMillis = minutes?.let { System.currentTimeMillis() + it * 60_000L }
                    ttsMessage = minutes?.let { "Autoapagado en ${it} min" } ?: "Autoapagado desactivado"
                },
